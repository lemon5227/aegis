...
| API | 说明 | 优先级 |
|-----|------|--------|
| `GetMyPosts(pubkey, limit)` | 获取用户发布的所有帖子 | 中 |
...
2. **My Posts**：当前从 localStorage 读取，需要后端实现 `GetMyPosts` API 来获取真实数据。
...
6. **窗口大小**：已修改 main.go，初始窗口大小改为 1200x800，背景色改为浅色。
